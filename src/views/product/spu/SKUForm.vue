  ...
  <div>
    <el-form label-width="120px">
      ...
      <el-form-item label="平台属性">
        <el-form inline>
          <el-form-item v-for="al in attrList" :key="al.id" :label="al.attrName">
            <el-select v-model="al.attrIdAndValue">
              <el-option v-for="vl in al.attrValueList" :key="vl.id" :label="vl.valueName" :value="`${al.id}:${vl.id}`">
              </el-option>
            </el-select>
          </el-form-item>
        </el-form>
      ...
      <el-form-item label="销售属性">
        <el-form inline>
          <el-form-item v-for="sal in  saleAttrList " :key="sal.id" :label="sal.saleAttrName">
            <el-select v-model="sal.saleAttrIdAndValue">
              <el-option v-for="ssavl in  sal.spuSaleAttrValueList " :key="ssavl.id" :label="ssavl.saleAttrValueName"
      ...
      <el-form-item label="图片名称">
        <el-table :data="imgList" border ref="tableRef">
          ...
          <el-table-column label="操作">
            <template v-slot="{ row }">
              <el-button type="primary" size="default" @click="handler(row)">设为默认</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
...
const SKUParams = reactive({ category3Id: '', spuId: '', tmId: '', skuName: '', price: '', weight: '', skuDesc: '', skuAttrValueList: [], skuSaleAttrValueList: [], skuDefaultImg: '' })
const tableRef = ref()
const initAddData = async (C1ID: number | string, C2ID: number | string, row: SPU) => {
...
const save = () => {
  SKUParams.skuAttrValueList = attrList.value.filter(item => item.attrIdAndValue).reduce((prev: any, next: any) => {
    let [valueId, valueName] = next.attrIdAndValue.split(':')
    prev.push({ valueId, valueName })
    return prev
  }, [])
  SKUParams.skuSaleAttrValueList = saleAttrList.value.filter(item => item.saleAttrIdAndValue).reduce((prev: any, next: any) => {

<template>
  <el-card shadow="always" :body-style="{ padding: '20px' }">
    <el-table :data="SKUList" border style="margin-bottom: 20px;">
      ...
      <el-table-column label="名称" width="320" prop="skuName" show-overflow-tooltip></el-table-column>
      <el-table-column label="描述" width="320" prop="skuDesc" show-overflow-tooltip></el-table-column>
      <el-table-column label="图片" width="300">
        <template v-slot="{ row }">
          <el-image :src="row.skuDefaultImg" fit="scale-down" :lazy="true" style="max-height:200px"></el-image>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="320" fixed="right" align="center">
        <template v-slot="{ row }">
          <el-button type="primary" size="small" :icon="row.isSale == 1 ? 'Download' : 'Upload'"
            @click="onOrCancleSale(row)"></el-button>
          <el-button type="warning" size="small" icon="Edit" @click="updateSKU"></el-button>
          <el-button type="info" size="small" icon="InfoFilled" @click="viewSKU(row.id)"></el-button>
          <el-popconfirm :title="`你确认要删除${row.skuName}吗？`" icon="Delete" width="200px" @confirm="deleteSKU(row.id)">
            <template #reference>
    ...
    <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[10, 20, 30]" background
      layout=" prev, pager, next, jumper,->,sizes,total" :total="total" @size-change="sizeChange"
      @current-change="getData" />
    <el-drawer title="商品详情" v-model="drawerVisible" direction="rtl" size="34%">
      <el-row :gutter="20">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="16" :offset="0">
          <el-tag type="danger" v-for="s in SKUInfo.skuAttrValueList" :key="s.id">{{ s.valueName }}</el-tag>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="7" :offset="1">销售属性</el-col>
        <el-col :span="16" :offset="0">
          <el-tag type="success" v-for="s in SKUInfo.skuSaleAttrValueList" :key="s.id">{{ s.saleAttrValueName }}</el-tag>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="7" :offset="1">商品图片</el-col>
        <el-col :span="16" :offset="0">
          <el-carousel height="150px" direction="horizontal" :autoplay="true" :interval="3000" :loop="true"
...
import { reqSKUList, reqOnSale, reqCancleSale, reqSKUInfo, reqDeleteSKU } from '@/api/product/sku/index';

  ...
  <el-card shadow="always" :body-style="{ padding: '20px' }">
    <el-table :data="SKUList" border style="margin-bottom: 20px">
      ...
      <el-table-column label="操作" width="320" fixed="right" align="center">
        <template v-slot="{ row }">
          <el-button type="primary" size="small" :icon="row.isSale == 1 ? 'Download' : 'Upload'" v-has="`btn.Sku.updown`"
            @click="onOrCancleSale(row)"></el-button>
          <el-button type="warning" size="small" icon="Edit" v-has="`btn.Sku.update`" @click="updateSKU"></el-button>
          <el-button type="info" size="small" icon="InfoFilled" v-has="`btn.Sku.detail`"
            @click="viewSKU(row.id)"></el-button>
          <el-popconfirm :title="`你确认要删除${row.skuName}吗？`" icon="Delete" width="200px" v-has="`btn.Spu.remove`"
    ...
    <el-drawer title="商品详情" v-model="drawerVisible" direction="rtl" size="34%">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="16" :offset="0">
          <el-tag type="danger" v-for="s in SKUInfo.skuAttrValueList" :key="s.id">
            {{ s.valueName }}
          </el-tag>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        ...
        <el-col :span="16" :offset="0">
          <el-tag type="success" v-for="s in SKUInfo.skuSaleAttrValueList" :key="s.id">
            {{ s.saleAttrValueName }}
          </el-tag>
        </el-col>
      </el-row>

      ...
      <div v-show="scene == 0">
        ...
        <el-table :data="attrList" border stripe style="margin: 20px 0">
          ...
          <el-table-column label="操作">
            <template #="{ row }">
              <el-button type="warning" size="small" icon="Edit" @click="attrEdit(row)"></el-button>
              <el-popconfirm :title="`你确认要删除${row.attrName}吗？`" icon="Delete" width="250px" @confirm="attrDelete(row.id)">
                <template #reference>
      ...
      <div v-show="scene == 1">
        ...
        <el-table :data="attrParams.attrValueList" border style="margin-bottom: 20px">
          ...
          <el-table-column label="属性值名称">
            <template #="{ row, $index }">
              <div v-if="row.viewable" @click="attrValueEdit(row, $index)">{{ row.valueName }}</div>
        ...
        <el-button type="primary" @click="addOrUpadateAttr"
